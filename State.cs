			...
			using (XmlReader logReader = XmlReader.Create(logFilePath))
			{
				while (logReader.Read())
				{
					if (logReader.IsStartElement())	// if we have a start tag
					{
						...
						ItemId id;			// we will attempt to parse item names to get the corresponding ItemID enum value
...
*/
							...
							case "gold_update":
								...
								break;
							case "purchase":
								// this indicates that the hero has just purchased an item from one of the stores
								// will usually co-occur with an inventory_update and a gold_update
								...
								// create a new status change event
								ItemPurchase purchaseEvent = new ItemPurchase();

								// read in the name of the item
								attribute = logReader["item"];
								if (attribute == null) break;
								if (Enum.TryParse(attribute, out id) == true) {
									purchaseEvent.NewItem = new Item(id);
								}
								else {
								...
								target.AddDiff(Tick, purchaseEvent);
								break;
							case "inventory_update":
								...
								if (attribute == null) break;
								if (Enum.TryParse(attribute, out id) == true) {
									inventoryEvent.Contents = new Item(id);
								}
								else {
									Console.WriteLine("Unrecognized item name: " + attribute);
									break;
