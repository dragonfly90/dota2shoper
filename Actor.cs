										...
										if (logReader.IsStartElement()) {
											if  (logReader.Name == "hero") { // reached data for another hero
												...
												if (n >= 0 && logReader.Read()) {
													...
													RecommendedItemTable[n] = new ItemId[itemList.Length];
													int i = 0;
													foreach (string item in itemList) {
														if (Enum.TryParse(item, out id) == true) {
															RecommendedItemTable[n][i++] = id;
														}
													}
												}
											}
...
//							Console.WriteLine("");
		...
		public void SetSlot(int index, ItemId id, int charges) {
